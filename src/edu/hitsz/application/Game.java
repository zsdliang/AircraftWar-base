...
public class Game extends JPanel {
     ...
     * 指示子弹的发射、敌机的产生频率
     */
    private int cycleDuration = 500;//敌机刷新频率和英雄机射击频率
    private int cycleTime = 0;
    private int cycleDuration_EnemyShoot = 2000;///////////////精英机射击频率
    ...
    private boolean enemyShootJudge() {
        ...
        if (cycleEnemyshoottime >= cycleDuration_EnemyShoot && cycleEnemyshoottime - enemyshootInterval < cycleEnemyshoottime) {
            cycleEnemyshoottime %= cycleDuration_EnemyShoot;
            return true;
        } else {
            return false;
        }
    ...
    private void crashCheckAction() {
        ...
        for(AbstractAircraft prop : props){
            if (heroAircraft.crash(prop)) {
                prop.vanish();
                if(prop instanceof Prop_Blood) {
                    System.out.println("hp+25");
                    heroAircraft.decreaseHp(-25);
                }
                else if(prop instanceof Prop_Bomb) {
                    System.out.println("bomb");
                }
                else if(prop instanceof Prop_Bullet) {
